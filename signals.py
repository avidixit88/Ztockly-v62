...
def compute_scalp_signal(
    ...
    *,
    ...
    # --- ORB levels (Opening Range Breakout) + score contribution (PRO) ---
    # Computes a simple ORB high/low over the first `orb_minutes` of the RTH session (09:30 ET).
    orb = False
    orb_side = None
    orb_high = None
    orb_low = None
    try:
        orb_high, orb_low = calc_orb_levels(df, interval_mins=interval_mins, orb_minutes=int(orb_minutes))
        if pro_mode and session == "OPENING" and orb_high is not None and orb_low is not None:
            prev_close = float(close_series.iloc[-2]) if len(close_series) >= 2 else float(close)
            # LONG ORB logic
            if close >= orb_high and prev_close < orb_high:
                orb = True
                orb_side = "LONG"
                long_points += 8
                chips.append("ORB break ↑")
            elif low <= orb_high <= close:
                orb = True
                orb_side = "LONG"
                long_points += 5
                chips.append("ORB retest ↑")
