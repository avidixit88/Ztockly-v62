...
def calc_vwap(df: pd.DataFrame) -> pd.Series:
    """Backward-compatible alias for cumulative VWAP."""
    return vwap(df)


def calc_session_vwap(df: pd.DataFrame, tz: str = "America/New_York", include_premarket: bool = True) -> pd.Series:
    """Session VWAP with proper ET day reset. If include_premarket=False, uses 09:30–16:00 ET bars."""
    if include_premarket:
        return session_vwap(df, tz=tz)

    if df is None or len(df) == 0:
        return pd.Series(dtype=float)

    d = df.copy()
    # Build a timezone-aware timestamp series from either 'time' column or index
    ...
    try:
        ...
        # Fall back to naive timestamps if tz ops fail
        pass
    ...
    def _is_rth(x):
        try:
            t = x.timetz() if hasattr(x, "timetz") else x.time()
            return (t >= pd.Timestamp("09:30").time()) and (t <= pd.Timestamp("16:00").time())
        except Exception:
            return True

    rth_mask = d["_ts"].apply(_is_rth)
    # compute session vwap only on RTH bars; forward-fill within each day for convenience
    v = pd.Series(index=d.index, dtype=float)
    if rth_mask.any():
        v_rth = session_vwap(d.loc[rth_mask].drop(columns=["_ts"], errors="ignore"), tz=tz)
        v.loc[rth_mask] = v_rth
        # forward-fill within each date
        try:
            dates = pd.to_datetime(ts).dt.date
            v = v.groupby(dates).ffill()
        except Exception:
            v = v.ffill()
    return v
